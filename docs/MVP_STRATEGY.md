...
| F13 | **Simulation** | A Simulation node previews the product, live in 3D, inside simulated places: an online shop product page, a TikTok feed with a shop card, a chat sticker (transparent PNG download) and a showroom steered from a phone. The phone remote is two-way: scanning the QR opens a no-sign-in page that sends tilt, drag, place and snapshot commands; the screen sends back what it shows and snapshots. Added 2026-09-24 at the user's request; more places can be added as layouts. |
