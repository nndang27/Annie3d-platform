...
| Downloads (PNG/JSON/MP4/WebM exports) never surfaced to the test driver | WebKit (Playwright issue #21892: no download event for `blob:` anchors) | App-side `downloadBlob` keeps a log of started downloads exposed on `window.__annie3d.downloads()`; the test helper `captureDownload` reads the bytes there on WebKit and uses the real download event elsewhere. The product code path is identical. |
...
| `window.__annie3d` read before boot finished on a fresh navigation | WebKit | Test waits for the app to boot (`waitForApp`). |
...
| **Mobile menu rendered as a collapsed, transparent list over the hero; its "Start a project" button was black-on-black** (reported by the founder from a phone screenshot of `/product`, 2026-09-13) | every engine, < 900 px | Root cause: the sticky header uses `backdrop-filter`, which makes it the containing block for `position: fixed` descendants, so the menu's `inset` resolved against the 64 px header and collapsed to 0 height. The menu is now `position: absolute; top: 100%` under the header with `height: calc(100dvh - var(--header-h))` and an opaque background; the CTA keeps white text. Regression test added to `tests/e2e/mobile.spec.ts`: menu box ≥ 60 % of the viewport, the element at the hero's centre belongs to the menu, CTA colour is white, Escape closes. The viewport matrix had not caught it because it never opened the menu. |
