...
async function conn(): Promise<Db> {
  const { db, client } = createDb(url!, { explicitTls: true });
...
async function fixture(db: Db) {
  const user = await one<{ id: string }>(
    db,
    sql`INSERT INTO users (name, email) VALUES ('Test', ${`t-${crypto.randomUUID()}@example.com`}) RETURNING id`,
  );
  const ws = await one<{ id: string }>(
    db,
    sql`INSERT INTO workspaces (name, created_by) VALUES ('WS', ${user.id}) RETURNING id`,
  );
  const board = await one<{ id: string }>(
    db,
    sql`INSERT INTO boards (workspace_id, title) VALUES (${ws.id}, 'B') RETURNING id`,
  );
...
async function run(db: Db, boardId: string, wsId: string) {
  return (
    await one<{ id: string }>(
      db,
      sql`INSERT INTO runs (board_id, workspace_id, scope, estimated_credits, idempotency_key) VALUES (${boardId}, ${wsId}, 'all', 5, uuidv7()) RETURNING id`,
    )
  ).id;
}
async function node(db: Db, boardId: string, wsId: string, kind = 'model3d', zKey = 'a0') {
  return one<{ id: string }>(
    db,
    sql`INSERT INTO board_nodes (id, board_id, workspace_id, kind, x, y, z_key) VALUES (uuidv7(), ${boardId}, ${wsId}, ${kind}, 0, 0, ${zKey}) RETURNING id`,
  );
}
...
d('database (live Neon test branch)', () => {
  ...
  it('never overdraws credits under 20 concurrent reservations', async () => {
    ...
    const acc = await one<{ balance: string; reserved: string }>(
      db,
      sql`SELECT balance, reserved FROM credit_accounts WHERE workspace_id = ${wsId}`,
    );
    expect([Number(acc.balance), Number(acc.reserved)]).toEqual([60, 60]);
    for (let i = 0; i < 12; i++) await settle(db, wsId, runIds[i]!, 5, 3);
    const after = await one<{ balance: string; reserved: string }>(
      db,
      sql`SELECT balance, reserved FROM credit_accounts WHERE workspace_id = ${wsId}`,
    );
  ...
  it('applies a payment grant once even when the webhook arrives twice at the same time', async () => {
    ...
    const r = await Promise.allSettled([
      grant(a, wsId, 100, 'purchase', ref, null),
      grant(b, wsId, 100, 'purchase', ref, null),
    ]);
    const acc = await one<{ balance: string }>(
      db,
      sql`SELECT balance FROM credit_accounts WHERE workspace_id = ${wsId}`,
    );
  ...
  it('keeps the ledger append-only but lets a workspace delete cascade', async () => {
    ...
    await expectPgError(
      db.execute(sql`UPDATE credit_entries SET amount = 999 WHERE workspace_id = ${wsId}`),
      '23001',
    );
    ...
    const left = await one<{ n: number }>(
      db,
      sql`SELECT count(*)::int AS n FROM credit_entries WHERE workspace_id = ${wsId}`,
    );
  ...
  it('rejects a negative balance at the table level', async () => {
    ...
    await expectPgError(
      db.execute(sql`UPDATE credit_accounts SET balance = -1 WHERE workspace_id = ${wsId}`),
      '23514',
    );
  });
  ...
  it('dedupes ready assets by content hash per workspace', async () => {
    ...
    const ins = (key: string, status: string) =>
      db.execute(
        sql`INSERT INTO assets (workspace_id, kind, mime, byte_size, sha256, bucket, storage_key, status) VALUES (${wsId}, 'image', 'image/png', 10, ${sha}, 'uploads', ${key}, ${status})`,
      );
    ...
    await expectPgError(
      db.execute(
        sql`INSERT INTO assets (workspace_id, kind, mime, byte_size, sha256, bucket, storage_key) VALUES (${wsId}, 'image', 'image/png', 10, 'not-a-hash', 'uploads', ${`k-${crypto.randomUUID()}`})`,
      ),
      '23514',
    );
  });
  ...
  it('enforces live edge uniqueness, no self loops, and object-shaped settings', async () => {
    ...
    const edge = () =>
      db.execute(
        sql`INSERT INTO board_edges (id, board_id, workspace_id, source_node_id, target_node_id, target_port) VALUES (uuidv7(), ${boardId}, ${wsId}, ${a.id}, ${b.id}, 'images')`,
      );
    await edge();
    ...
    await expectPgError(
      db.execute(
        sql`INSERT INTO board_edges (id, board_id, workspace_id, source_node_id, target_node_id, target_port) VALUES (uuidv7(), ${boardId}, ${wsId}, ${a.id}, ${a.id}, 'images')`,
      ),
      '23514',
    );
    await expectPgError(
      db.execute(sql`UPDATE board_nodes SET settings = '[1,2]'::jsonb WHERE id = ${a.id}`),
      '23514',
    );
  });
  ...
  it('only accepts a current version that belongs to the node', async () => {
    ...
    const v = await one<{ id: string }>(
      db,
      sql`INSERT INTO node_versions (node_id, board_id, workspace_id, version_no, source) VALUES (${b.id}, ${boardId}, ${wsId}, 1, 'run') RETURNING id`,
    );
    await expectPgError(
      db.execute(sql`UPDATE board_nodes SET current_version_id = ${v.id} WHERE id = ${a.id}`),
      '23503',
    );
    await db.execute(sql`UPDATE board_nodes SET current_version_id = ${v.id} WHERE id = ${b.id}`);
    await expectPgError(
      db.execute(
        sql`INSERT INTO node_versions (node_id, board_id, workspace_id, version_no, source) VALUES (${b.id}, ${boardId}, ${wsId}, 1, 'run')`,
      ),
      '23505',
    );
  });
  ...
  it('makes op batches idempotent per board and orders z-keys byte-wise', async () => {
    ...
    await db.execute(
      sql`INSERT INTO board_ops (board_id, seq, op_id, ops) VALUES (${boardId}, 1, ${opId}, '[]'::jsonb)`,
    );
    await expectPgError(
      db.execute(
        sql`INSERT INTO board_ops (board_id, seq, op_id, ops) VALUES (${boardId}, 2, ${opId}, '[]'::jsonb)`,
      ),
      '23505',
    );
    for (const k of ['a0V', 'Zz', 'a0', 'a1']) await node(db, boardId, wsId, 'note', k);
    const order = (
      await db.execute<{ z_key: string }>(
        sql`SELECT z_key FROM board_nodes WHERE board_id = ${boardId} ORDER BY z_key`,
      )
    ).rows.map((r) => r.z_key);
  ...
  it('maintains updated_at in the database', async () => {
    ...
    const before = await one<{ updated_at: Date }>(
      db,
      sql`SELECT updated_at FROM boards WHERE id = ${boardId}`,
    );
