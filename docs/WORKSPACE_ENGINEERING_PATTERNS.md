...
| Debounced zoom for LOD | tldraw `getEfficientZoomLevel()` (above 500 shapes, LOD decisions use a debounced zoom while the camera moves) | Level-of-detail switches only after zoom settles, so pan/zoom never triggers re-renders | **Now.** Our zoom LOD (full / medium) reads a debounced zoom. Zoom is clamped to 25–200% (2026-09-24 user decision), so no text-only summary level exists. |
